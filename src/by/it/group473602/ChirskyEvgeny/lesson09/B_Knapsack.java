...
public class B_Knapsack {
    ...
    int getMaxWeight(InputStream stream) {
        ...
        for (int i = 1; i < count + 1; i++) {
            for (int w = 1; w < weight + 1; w++) {
                ...
                if (object[i - 1] <= w) {
                    maxWeight[w][i] = max(maxWeight[w][i], maxWeight[w - object[i - 1]][i - 1] + object[i - 1]);
                }
            }
        }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        InputStream stream = new FileInputStream(root + "by/it/group473602/ChirskyEvgeny/lesson09/dataB.txt");
